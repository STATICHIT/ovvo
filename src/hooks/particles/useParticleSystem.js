...
export function useParticleSystem(canvasRef) {
  ...
  const particleCountRef = useRef(400);
  ...
  useEffect(() => {
    ...
    const velocities = new Float32Array(particleCountRef.current);
    ...
    const alphas = new Float32Array(particleCountRef.current);
    for (let i = 0; i < particleCountRef.current; i++) {
    ...
    const animate = () => {
      ...
      if (particlesRef.current && velocitiesRef.current && alphasRef.current) {
        const positions = particlesRef.current.geometry.attributes.position.array;
        const alphas = alphasRef.current;
        const time = Date.now() * 0.001;

        for (let i = 0; i < particleCountRef.current; i++) {
          positions[i * 3] += velocitiesRef.current[i] * 0.016;
    ...
    const handleResize = () => {
      if (cameraRef.current && rendererRef.current) {
        ...
        rendererRef.current.setSize(window.innerWidth, 250);
      }
    };
